...
const POSTS=[
  ...
  {id:"p3",column:"prod",type:"reel",title:"Sua aluna grávida confia em você?",caption:"Você realmente sabe adaptar exercícios para gestantes?",tags:["gestante","provocativo"],assignee:"editor",createdBy:"editor",createdAt:"2026-02-19T08:00:00",updatedAt:"2026-02-22T16:00:00",scheduledDate:null,scheduledTime:null,engagement:null,notes:"Falta gravar",links:[],attachments:[],aiScore:72,aiSuggestion:"Score 72 — falta CTA forte."},
  {id:"p4",column:"edicao_revisao",type:"carrossel",title:"Checklist: 30 itens pra abrir studio",caption:"Abrir um studio sem checklist é como dar aula sem planejamento...",tags:["mba"],assignee:"editor",createdBy:"editor",createdAt:"2026-02-17T09:00:00",updatedAt:"2026-02-23T14:00:00",scheduledDate:null,scheduledTime:null,engagement:null,notes:"",links:[],attachments:[],aiScore:92,aiSuggestion:"Score 92 — excelente!"},
  {id:"p5",column:"agendado",type:"reel",title:"O instrutor que cobra R$50 vai falir",caption:"Eu sei que dói ouvir isso...\n💾 Salve e reveja seus números.",tags:["provocativo","mba"],assignee:"rafael",createdBy:"rafael",createdAt:"2026-02-15T10:00:00",updatedAt:"2026-02-24T08:00:00",scheduledDate:"2026-02-25",scheduledTime:"07:00",engagement:null,notes:"Pronto!",links:[],attachments:[],aiScore:95,aiSuggestion:"Score 95 — potencial máximo!"},
  {id:"p6",column:"publicado",type:"carrossel",title:"Neuromarketing no studio",caption:"85% das decisões são inconscientes...",tags:["mba","educativo"],assignee:"rafael",createdBy:"rafael",createdAt:"2026-02-10T08:00:00",updatedAt:"2026-02-20T18:00:00",scheduledDate:"2026-02-20",scheduledTime:"07:00",engagement:{likes:847,comments:123,saves:456,shares:89,reach:12400},notes:"",links:[],attachments:[],aiScore:88,aiSuggestion:"Saves acima da média!"},
  {id:"p7",column:"publicado",type:"reel",title:"Por que abri 3 studios e fechei 1",caption:"A história que ninguém conta...",tags:["storytelling","mba"],assignee:"rafael",createdBy:"rafael",createdAt:"2026-02-08T09:00:00",updatedAt:"2026-02-18T20:00:00",scheduledDate:"2026-02-18",scheduledTime:"18:00",engagement:{likes:1243,comments:287,saves:678,reach:23500,shares:156},notes:"",links:[],attachments:[],aiScore:91,aiSuggestion:"Melhor post do mês!"},
];
...
function CMSModal({modal,onClose,onSaveColumn,onSavePostType,onSavePillar,onSaveUser,onSaveSpecialDate}){
  ...
  return<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}}onClick={e=>e.target===e.currentTarget&&onClose()}}><div style={{background:T.bg,borderRadius:12,border:`1px solid ${T.border}`,padding:20,minWidth:320,maxWidth:400,animation:"scaleIn 0.2s"}}onClick={e=>e.stopPropagation()}}>
...
export default function App(){
  ...
  return<div style={{minHeight:"100vh",background:T.bg,fontFamily:T.font,color:T.text}}><style>{CSS}</style>
    <header style={{height:52,padding:"0 14px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${T.border}`,background:`${T.surface}ee`,backdropFilter:"blur(16px)",position:"sticky",top:0,zIndex:100}}>
      ...
      <div style={{flex:1,maxWidth:260,margin:"0 14px"}}><input value={search}onChange={e=>setSearch(e.target.value)}placeholder="🔍 Buscar..."style={{width:"100%",background:"rgba(255,255,255,0.03)",border:`1px solid ${T.border}`,borderRadius:6,padding:"5px 11px",color:T.text,fontSize:11.5,fontFamily:T.font,outline:"none"}}onFocus={e=>e.target.style.borderColor=T.accentBorder}onBlur={e=>e.target.style.borderColor=T.border}/></div>
      <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{display:"flex",gap:2}}>{postTypes.map(pt=><IconBtn key={pt.id}small active={filter.type===pt.id}onClick={()=>setFilter(f=>({type:f.type===pt.id?null:pt.id}))}>{pt.icon}</IconBtn>)}</div><div style={{width:1,height:18,background:T.border}}/><button title="Notificações"style={{width:32,height:32,borderRadius:6,border:`1px solid ${T.border}`,background:"transparent",color:T.textMuted,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>🔔</button><div style={{display:"flex",gap:2,background:"rgba(255,255,255,0.03)",borderRadius:5,padding:2}}>{users.map(u=><button key={u.id}onClick={()=>setUser(u)}style={{display:"flex",alignItems:"center",gap:3,padding:"3px 8px",borderRadius:4,fontSize:10.5,fontWeight:600,border:user.id===u.id?`1px solid ${u.color}50`:"1px solid transparent",background:user.id===u.id?`${u.color}12`:"transparent",color:user.id===u.id?u.color:T.textMuted,cursor:"pointer",fontFamily:T.font}}><Avatar user={u.id}users={users}size={14}/>{u.name}</button>)}</div></div>
    </header>

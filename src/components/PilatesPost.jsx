...
const COLUMNS=[{id:"idea",label:"Ideias",icon:"💡",color:"#FF6B35",desc:"Brainstorms"},{id:"draft",label:"Rascunho",icon:"✍️",color:"#F7C948",desc:"Em desenvolvimento"},{id:"review",label:"Revisão",icon:"👀",color:"#4ECDC4",desc:"Aprovação"},{id:"scheduled",label:"Agendado",icon:"📅",color:"#45B7D1",desc:"Pronto"},{id:"published",label:"Publicado",icon:"🚀",color:"#5DE8A0",desc:"No ar!"}];
const POST_TYPES=[{id:"reel",label:"Reel",icon:"🎬",color:"#FF6B35"},{id:"carrossel",label:"Carrossel",icon:"📑",color:"#F7C948"},{id:"static",label:"Imagem",icon:"🖼️",color:"#4ECDC4"},{id:"collab",label:"Collab",icon:"🤝",color:"#45B7D1"}];
...
const USERS=[{id:"rafael",name:"Rafael",role:"owner",avatar:"R",color:"#FF6B35"},{id:"editor",name:"Editor",role:"editor",avatar:"E",color:"#4ECDC4"}];
...
// --- POSTS DATA ---
const POSTS=[
  {id:"p1",column:"idea",type:"reel",title:"5 erros que quebram studios",caption:"",tags:["provocativo"],assignee:null,createdBy:"rafael",createdAt:"2026-02-20T10:00:00",updatedAt:"2026-02-20T10:00:00",scheduledDate:null,scheduledTime:null,engagement:null,notes:"",aiScore:null,aiSuggestion:"Tema em alta! Oportunidade de ouro."},
  {id:"p2",column:"idea",type:"carrossel",title:"O que Joseph Pilates diria hoje",caption:"",tags:["storytelling","educativo"],assignee:"editor",createdBy:"rafael",createdAt:"2026-02-20T14:00:00",updatedAt:"2026-02-21T09:00:00",scheduledDate:null,scheduledTime:null,engagement:null,notes:"",aiScore:null,aiSuggestion:"Carrossel histórico tem saves altos."},
  {id:"p3",column:"draft",type:"reel",title:"Sua aluna grávida confia em você?",caption:"Você realmente sabe adaptar exercícios para gestantes?",tags:["gestante","provocativo"],assignee:"editor",createdBy:"editor",createdAt:"2026-02-19T08:00:00",updatedAt:"2026-02-22T16:00:00",scheduledDate:null,scheduledTime:null,engagement:null,notes:"Falta gravar",aiScore:72,aiSuggestion:"Score 72 — falta CTA forte."},
  {id:"p4",column:"review",type:"carrossel",title:"Checklist: 30 itens pra abrir studio",caption:"Abrir um studio sem checklist é como dar aula sem planejamento...",tags:["mba"],assignee:"editor",createdBy:"editor",createdAt:"2026-02-17T09:00:00",updatedAt:"2026-02-23T14:00:00",scheduledDate:null,scheduledTime:null,engagement:null,notes:"",aiScore:92,aiSuggestion:"Score 92 — excelente!"},
  {id:"p5",column:"scheduled",type:"reel",title:"O instrutor que cobra R$50 vai falir",caption:"Eu sei que dói ouvir isso...\n💾 Salve e reveja seus números.",tags:["provocativo","mba"],assignee:"rafael",createdBy:"rafael",createdAt:"2026-02-15T10:00:00",updatedAt:"2026-02-24T08:00:00",scheduledDate:"2026-02-25",scheduledTime:"07:00",engagement:null,notes:"Pronto!",aiScore:95,aiSuggestion:"Score 95 — potencial máximo!"},
  {id:"p6",column:"published",type:"carrossel",title:"Neuromarketing no studio",caption:"85% das decisões são inconscientes...",tags:["mba","educativo"],assignee:"rafael",createdBy:"rafael",createdAt:"2026-02-10T08:00:00",updatedAt:"2026-02-20T18:00:00",scheduledDate:"2026-02-20",scheduledTime:"07:00",engagement:{likes:847,comments:123,saves:456,shares:89,reach:12400},notes:"",aiScore:88,aiSuggestion:"Saves acima da média!"},
  {id:"p7",column:"published",type:"reel",title:"Por que abri 3 studios e fechei 1",caption:"A história que ninguém conta...",tags:["storytelling","mba"],assignee:"rafael",createdBy:"rafael",createdAt:"2026-02-08T09:00:00",updatedAt:"2026-02-18T20:00:00",scheduledDate:"2026-02-18",scheduledTime:"18:00",engagement:{likes:1243,comments:287,saves:678,reach:23500,shares:156},notes:"",aiScore:91,aiSuggestion:"Melhor post do mês!"},
];
  ...
  return<div style={{padding:"18px 20px",animation:"fadeIn 0.3s"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        ...
        <button onClick={()=>newStory()}style={{padding:"6px 14px",borderRadius:8,fontSize:12,fontWeight:700,border:"none",background:`linear-gradient(135deg,${T.accent},${T.yellow})`,color:T.bg,cursor:"pointer",fontFamily:T.font}}>+ Nova Sequência</button>
      </div>
    </div>
...
// Post Card
function PostCard({post,onEdit,onDragStart,isDragging}){const[h,setH]=useState(false);const ti=POST_TYPES.find(t=>t.id===post.type);const tags=post.tags.map(t=>PILLARS.find(p=>p.id===t)).filter(Boolean);
...
function KanbanCol({col,posts,onEdit,onDragStart,onDrop,dp,onNew}){const[dg,setDg]=useState(false);
  return<div onDragOver={e=>{e.preventDefault();setDg(true)}}onDragLeave={()=>setDg(false)}onDrop={e=>{e.preventDefault();setDg(false);onDrop(col.id)}}style={{minWidth:265,maxWidth:265,display:"flex",flexDirection:"column",borderRadius:11,border:dg&&dp?.column!==col.id?`1px dashed ${T.accentBorder}`:"1px solid transparent",transition:"all 0.2s",background:dg&&dp?.column!==col.id?"rgba(255,107,53,0.03)":"transparent"}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"0 4px 5px"}}>{posts.map(p=><PostCard key={p.id}post={p}onEdit={onEdit}onDragStart={onDragStart}isDragging={dp?.id===p.id}/>)}</div>
    {col.id!=="published"&&<button onClick={()=>onNew(col.id)}style={{margin:"2px 4px 7px",padding:8,background:"transparent",border:`1px dashed ${T.border}`,borderRadius:8,color:T.textDim,fontSize:11,cursor:"pointer",fontFamily:T.font}}onMouseEnter={e=>{e.target.style.borderColor=T.accentBorder;e.target.style.color=T.accent}}onMouseLeave={e=>{e.target.style.borderColor=T.border;e.target.style.color=T.textDim}}>+ Novo</button>}
  </div>}

// Post Editor (simplified)
function PostEditor({post,onSave,onClose,onDelete}){const[f,setF]=useState({...post});const up=(k,v)=>setF(p=>({...p,[k]:v}));
  return<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(6px)"}}onClick={e=>{if(e.target===e.currentTarget)onClose()}}><div style={{width:"min(680px,95vw)",maxHeight:"85vh",background:T.surface,borderRadius:16,display:"flex",flexDirection:"column",border:`1px solid ${T.border}`,animation:"scaleIn 0.2s",overflow:"hidden"}}>
    <div style={{padding:"12px 18px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between"}}><Badge color={COLUMNS.find(c=>c.id===f.column)?.color}>{COLUMNS.find(c=>c.id===f.column)?.icon} {COLUMNS.find(c=>c.id===f.column)?.label}</Badge><div style={{display:"flex",gap:4}}><AIS score={f.aiScore}size="lg"/><IconBtn onClick={()=>{if(confirm("Excluir?")){onDelete(f.id);onClose()}}}>🗑️</IconBtn><IconBtn onClick={onClose}>✕</IconBtn></div></div>
    <div style={{flex:1,overflowY:"auto",padding:18}}>
      <input value={f.title}onChange={e=>up("title",e.target.value)}placeholder="Título..."style={{width:"100%",background:"transparent",border:"none",color:T.text,fontSize:17,fontWeight:700,fontFamily:T.font,outline:"none",marginBottom:12,borderBottom:`1px solid ${T.border}`,paddingBottom:8}}/>
      <div style={{display:"flex",gap:4,marginBottom:10,flexWrap:"wrap"}}>{POST_TYPES.map(pt=><button key={pt.id}onClick={()=>up("type",pt.id)}style={{padding:"3px 9px",borderRadius:6,fontSize:11,fontWeight:600,border:f.type===pt.id?`1px solid ${pt.color}60`:`1px solid ${T.border}`,background:f.type===pt.id?`${pt.color}15`:"transparent",color:f.type===pt.id?pt.color:T.textMuted,cursor:"pointer",fontFamily:T.font}}>{pt.icon}{pt.label}</button>)}</div>
      <div style={{display:"flex",gap:3,marginBottom:10,flexWrap:"wrap"}}>{PILLARS.map(p=><button key={p.id}onClick={()=>setF(prev=>({...prev,tags:prev.tags.includes(p.id)?prev.tags.filter(x=>x!==p.id):[...prev.tags,p.id]}))}style={{padding:"3px 9px",borderRadius:6,fontSize:11,fontWeight:600,border:f.tags.includes(p.id)?`1px solid ${p.color}50`:`1px solid ${T.border}`,background:f.tags.includes(p.id)?`${p.color}15`:"transparent",color:f.tags.includes(p.id)?p.color:T.textDim,cursor:"pointer",fontFamily:T.font}}>#{p.label}</button>)}</div>
      <textarea value={f.caption}onChange={e=>up("caption",e.target.value)}placeholder="Legenda..."rows={4}style={{width:"100%",background:"rgba(255,255,255,0.02)",border:`1px solid ${T.border}`,borderRadius:8,padding:"10px 12px",color:T.text,fontSize:12.5,fontFamily:T.font,lineHeight:1.6,outline:"none",resize:"vertical",marginBottom:10}}/>
      <div style={{display:"flex",gap:10,marginBottom:10}}><input type="date"value={f.scheduledDate||""}onChange={e=>up("scheduledDate",e.target.value)}style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${T.border}`,borderRadius:7,padding:"7px 10px",color:T.text,fontSize:12,fontFamily:T.font,outline:"none",colorScheme:"dark"}}/><input type="time"value={f.scheduledTime||""}onChange={e=>up("scheduledTime",e.target.value)}style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${T.border}`,borderRadius:7,padding:"7px 10px",color:T.text,fontSize:12,fontFamily:T.font,outline:"none",colorScheme:"dark"}}/></div>
      <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{COLUMNS.map(c=><button key={c.id}onClick={()=>up("column",c.id)}style={{padding:"4px 10px",borderRadius:6,fontSize:11,fontWeight:600,border:f.column===c.id?`1px solid ${c.color}60`:`1px solid ${T.border}`,background:f.column===c.id?`${c.color}15`:"transparent",color:f.column===c.id?c.color:T.textMuted,cursor:"pointer",fontFamily:T.font}}>{c.icon}{c.label}</button>)}</div>
    </div>
    <div style={{padding:"10px 18px",borderTop:`1px solid ${T.border}`,display:"flex",justifyContent:"flex-end",gap:6}}><button onClick={onClose}style={{padding:"6px 14px",borderRadius:8,fontSize:12,border:`1px solid ${T.border}`,background:"transparent",color:T.textMuted,cursor:"pointer",fontFamily:T.font}}>Cancelar</button><button onClick={()=>{onSave(f);onClose()}}style={{padding:"6px 18px",borderRadius:8,fontSize:12,fontWeight:700,border:"none",background:`linear-gradient(135deg,${T.accent},${T.yellow})`,color:T.bg,cursor:"pointer",fontFamily:T.font}}>Salvar</button></div>
  </div></div>}

// Calendar
function CalView({posts,onEdit,wo,setWo}){const base=new Date(today);base.setDate(base.getDate()+wo*7);const wd=getWeekDates(base);
  return<div style={{padding:"16px 18px",animation:"fadeIn 0.3s"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><IconBtn onClick={()=>setWo(w=>w-1)}>←</IconBtn><h2 style={{fontSize:15,fontWeight:800,color:T.text,margin:0}}>{wd[0].toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})}—{wd[6].toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})}</h2><IconBtn onClick={()=>setWo(w=>w+1)}>→</IconBtn>{wo!==0&&<button onClick={()=>setWo(0)}style={{fontSize:10,color:T.accent,background:T.accentGlow,border:`1px solid ${T.accentBorder}`,padding:"2px 7px",borderRadius:5,cursor:"pointer",fontFamily:T.font,fontWeight:600}}>Hoje</button>}</div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:6}}>{wd.map((d,i)=>{const isT=d.toDateString()===today.toDateString();return<div key={`h${i}`}style={{textAlign:"center",paddingBottom:6,borderBottom:isT?`2px solid ${T.accent}`:`1px solid ${T.border}`}}><div style={{fontSize:10,fontWeight:700,color:isT?T.accent:T.textMuted,textTransform:"uppercase"}}>{DAY_NAMES[i]}</div><div style={{fontSize:15,fontWeight:800,color:isT?T.accent:T.text,fontFamily:T.mono}}>{d.getDate()}</div></div>})}{wd.map((d,i)=>{const ds=d.toISOString().split("T")[0];const dp=posts.filter(p=>p.scheduledDate===ds);const tpl=WEEK_TPL.find(t=>t.day===i);const isT=d.toDateString()===today.toDateString();return<div key={`c${i}`}style={{minHeight:120,borderRadius:9,padding:6,background:isT?"rgba(255,107,53,0.04)":T.card,border:isT?`1px solid ${T.accentBorder}`:`1px solid ${T.border}`,opacity:d<today&&!isT?0.5:1}}>{tpl&&!dp.length&&<div style={{padding:"5px 7px",borderRadius:6,border:`1px dashed ${T.borderHover}`}}><div style={{fontSize:9,color:T.textDim}}>{tpl.time}</div><div style={{fontSize:10,color:T.textMuted,fontWeight:600}}>{POST_TYPES.find(t=>t.id===tpl.type)?.icon}{tpl.label}</div></div>}{dp.map(p=>{const ti=POST_TYPES.find(t=>t.id===p.type);return<div key={p.id}onClick={()=>onEdit(p)}style={{padding:"5px 7px",borderRadius:6,cursor:"pointer",background:`${ti?.color}10`,border:`1px solid ${ti?.color}25`,marginBottom:3}}><span style={{fontSize:9,color:T.textMuted}}>{p.scheduledTime}</span><div style={{fontSize:10.5,fontWeight:650,color:T.text,marginTop:1}}>{p.title.length>28?p.title.substring(0,28)+"...":p.title}</div></div>})}</div>})}</div></div>}
...
export default function App(){
  const[posts,setPosts]=useState(POSTS);const[view,setView]=useState("board");const[user,setUser]=useState(USERS[0]);const[editPost,setEditPost]=useState(null);const[dp,setDp]=useState(null);const[showAI,setShowAI]=useState(false);const[wo,setWo]=useState(0);const[filter,setFilter]=useState({type:null});const[search,setSearch]=useState("");
  ...
  const newP=useCallback(colId=>{setEditPost({id:genId(),column:colId,type:"reel",title:"",caption:"",tags:[],assignee:user.id,createdBy:user.id,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),scheduledDate:null,scheduledTime:null,engagement:null,notes:"",aiScore:null,aiSuggestion:null})},[user.id]);
  ...
  const navs=[{id:"board",l:"Board",i:"◻"},{id:"calendar",l:"Calendário",i:"◫"},{id:"stories",l:"Stories",i:"📱"},{id:"metrics",l:"Métricas",i:"📊"},{id:"trends",l:"Tendências",i:"🔥"},{id:"crm",l:"CRM",i:"🤝"},{id:"hooks",l:"Ganchos",i:"🎣"}];
  return<div style={{minHeight:"100vh",background:T.bg,fontFamily:T.font,color:T.text}}><style>{CSS}</style>
    <header style={{height:50,padding:"0 12px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${T.border}`,background:`${T.surface}ee`,backdropFilter:"blur(16px)",position:"sticky",top:0,zIndex:100}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:26,height:26,borderRadius:6,background:`linear-gradient(135deg,${T.accent},${T.yellow})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:900,color:T.bg,fontFamily:T.mono}}>P</div><span style={{fontSize:13,fontWeight:800,background:`linear-gradient(135deg,${T.accent},${T.yellow})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>PilatesPost</span></div><div style={{width:1,height:18,background:T.border}}/><div style={{display:"flex",gap:1,background:"rgba(255,255,255,0.03)",borderRadius:6,padding:2}}>{navs.map(v=><button key={v.id}onClick={()=>setView(v.id)}style={{padding:"3px 8px",borderRadius:4,fontSize:10.5,fontWeight:600,border:"none",cursor:"pointer",fontFamily:T.font,background:view===v.id?T.accentGlow:"transparent",color:view===v.id?T.accent:T.textMuted,transition:"all 0.15s"}}>{v.i} {v.l}</button>)}</div></div>
      <div style={{flex:1,maxWidth:250,margin:"0 12px"}}><input value={search}onChange={e=>setSearch(e.target.value)}placeholder="🔍 Buscar..."style={{width:"100%",background:"rgba(255,255,255,0.03)",border:`1px solid ${T.border}`,borderRadius:6,padding:"4px 10px",color:T.text,fontSize:11.5,fontFamily:T.font,outline:"none"}}onFocus={e=>e.target.style.borderColor=T.accentBorder}onBlur={e=>e.target.style.borderColor=T.border}/></div>
      <div style={{display:"flex",alignItems:"center",gap:5}}><div style={{display:"flex",gap:2}}>{POST_TYPES.map(pt=><IconBtn key={pt.id}small active={filter.type===pt.id}onClick={()=>setFilter(f=>({type:f.type===pt.id?null:pt.id}))}>{pt.icon}</IconBtn>)}</div><div style={{width:1,height:18,background:T.border}}/><button onClick={()=>setShowAI(true)}style={{background:`linear-gradient(135deg,${T.accent},${T.yellow})`,border:"none",padding:"4px 11px",borderRadius:7,color:T.bg,fontWeight:800,cursor:"pointer",fontSize:11,fontFamily:T.font,display:"flex",alignItems:"center",gap:3,boxShadow:"0 2px 10px rgba(255,107,53,0.25)"}}>🧠 IA</button><div style={{width:1,height:18,background:T.border}}/><div style={{display:"flex",gap:2,background:"rgba(255,255,255,0.03)",borderRadius:5,padding:2}}>{USERS.map(u=><button key={u.id}onClick={()=>setUser(u)}style={{display:"flex",alignItems:"center",gap:3,padding:"2px 7px",borderRadius:4,fontSize:10,fontWeight:600,border:user.id===u.id?`1px solid ${u.color}50`:"1px solid transparent",background:user.id===u.id?`${u.color}12`:"transparent",color:user.id===u.id?u.color:T.textMuted,cursor:"pointer",fontFamily:T.font}}><Avatar user={u.id}size={13}/>{u.name}</button>)}</div></div>
    </header>
    {(view==="board"||view==="calendar")&&<div style={{padding:"0 12px"}}>{AI_ALERTS.slice(0,2).map((a,i)=><div key={a.id}style={{padding:"7px 12px",marginTop:5,borderRadius:8,background:a.urgency==="alta"?"rgba(255,107,53,0.04)":"rgba(247,201,72,0.03)",border:`1px solid ${a.urgency==="alta"?T.accentBorder:"rgba(247,201,72,0.15)"}`,display:"flex",alignItems:"center",gap:8,fontSize:11,animation:`fadeIn 0.3s ease ${i*0.1}s both`}}><span style={{fontSize:13}}>{a.icon}</span><span style={{color:T.textMuted,flex:1}}>{a.text}</span><button style={{background:T.accentGlow,border:`1px solid ${T.accentBorder}`,color:T.accent,padding:"2px 8px",borderRadius:5,fontSize:9.5,cursor:"pointer",fontFamily:T.font,fontWeight:600,whiteSpace:"nowrap"}}>{a.act}</button></div>)}</div>}
    <main>{view==="board"&&<div style={{display:"flex",gap:7,padding:"8px 8px",overflowX:"auto",minHeight:"calc(100vh - 100px)"}}>{COLUMNS.map(c=><KanbanCol key={c.id}col={c}posts={filtered.filter(p=>p.column===c.id)}onEdit={setEditPost}onDragStart={setDp}onDrop={drop}dp={dp}onNew={newP}/>)}</div>}{view==="calendar"&&<CalView posts={posts}onEdit={setEditPost}wo={wo}setWo={setWo}/>}{view==="stories"&&<StoriesView/>}{view==="metrics"&&<MetricsView/>}{view==="trends"&&<TrendsView/>}{view==="crm"&&<CRMView/>}{view==="hooks"&&<HooksView/>}</main>
    {editPost&&<PostEditor post={editPost}onSave={p=>{save(p);setEditPost(null)}}onClose={()=>setEditPost(null)}onDelete={id=>{del(id);setEditPost(null)}}/>}
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,height:28,background:`${T.surface}f0`,backdropFilter:"blur(12px)",borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",gap:14,fontSize:9.5,color:T.textMuted,zIndex:90}}>
      <span>{posts.length} posts</span><span>💡{posts.filter(p=>p.column==="idea").length}</span><span>✍️{posts.filter(p=>p.column==="draft").length}</span><span>📅{posts.filter(p=>p.column==="scheduled").length}</span><span>🚀{posts.filter(p=>p.column==="published").length}</span><span style={{color:T.border}}>|</span><span><span style={{color:user.color,fontWeight:700}}>{user.name}</span> {user.role==="owner"?"👑":"✏️"}</span>
    </div>
  </div>}

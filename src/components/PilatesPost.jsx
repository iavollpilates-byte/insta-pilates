...
function PostEditor({post,onSave,onClose,onDelete,visibleFields=[],columns,postTypes,pillars,users}){
  ...
  return<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(6px)"}}onClick={e=>{if(e.target===e.currentTarget)onClose()}}><div style={{width:"min(700px,95vw)",maxHeight:"90vh",background:T.surface,borderRadius:16,border:`1px solid ${T.border}`,boxShadow:"0 24px 60px rgba(0,0,0,0.2)",animation:"scaleIn 0.2s",overflow:"hidden"}}>
  ...
  return<div data-theme={theme}style={{minHeight:"100vh",background:T.bg,fontFamily:T.font,color:T.text}}><style>{CSS}</style>
    <header style={{height:48,padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${T.border}`,background:T.surface,backdropFilter:"blur(12px)",position:"sticky",top:0,zIndex:100,boxShadow:"0 1px 0 0 "+T.border}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        ...
        <button style={{padding:"5px 12px",borderRadius:6,fontSize:11,fontWeight:600,border:`1px solid ${T.accentBorder}`,background:"transparent",color:T.accent,cursor:"pointer",fontFamily:T.font}}>Importar</button>
      </div>

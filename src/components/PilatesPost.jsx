...
function KanbanCol({col,posts,onEdit,onDragStart,onDrop,dp,onNew,users,postTypes,pillars,showLowStockAlert,agendadoMinCards}){const[dg,setDg]=useState(false);
...
function CMSView({cms,setCms,columns,postTypes,pillars,users,cardFormFieldIds,setCardFormFieldIds,setPosts}){
  ...
  const pageBackgrounds=cms.pageBackgrounds||{board:"",calendar:"",cms:""};const setPageBg=(page,val)=>{setCms(prev=>({...prev,pageBackgrounds:{...(prev.pageBackgrounds||{}),[page]:val||""}}))};
  const alertConfig=cms.alertConfig||{scheduledEnabled:true,scheduledDaysAfter:1,draftEnabled:true,draftDaysStale:7,specialDatesDaysBefore:12,agendadoMinEnabled:true,agendadoMinCards:3};const setAlert=(key,val)=>{setCms(prev=>({...prev,alertConfig:{...prev.alertConfig,[key]:val}}))};
  ...
  return<div style={{padding:"24px 20px",animation:"fadeIn 0.3s",maxWidth:620,overflowY:"auto",maxHeight:"calc(100vh - 90px)"}}>
    ...
    {section("Cor de fundo por página",<><p style={{fontSize:12,color:T.textMuted,marginBottom:10}}>Defina a cor de fundo da tela para cada página. Deixe em branco para usar o padrão (tema + rosa leve na conta Clara). Use hex (ex: #0B0B1A) ou rgba (ex: rgba(255,182,193,0.06)).</p><div style={{display:"flex",flexDirection:"column",gap:10}}>{[{id:"board",label:"Board"},{id:"calendar",label:"Calendário"},{id:"cms",label:"CMS"}].map(({id,label})=>(<div key={id}style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:T.card,borderRadius:10,border:`1px solid ${T.border}`}}><span style={{fontSize:13,fontWeight:600,color:T.text,minWidth:90}}>{label}</span><input type="color"value={/^#[0-9A-Fa-f]{6}$/.test(pageBackgrounds[id])?pageBackgrounds[id]:"#0B0B1A"}onChange={e=>setPageBg(id,e.target.value)}style={{width:36,height:36,borderRadius:8,border:`1px solid ${T.border}`,padding:2,cursor:"pointer"}}/><input type="text"value={pageBackgrounds[id]||""}onChange={e=>setPageBg(id,e.target.value)}placeholder="Padrão"style={{flex:1,background:T.surface,border:`1px solid ${T.border}`,borderRadius:6,padding:"6px 10px",color:T.text,fontSize:12,fontFamily:T.mono}}/></div>))}</div></>)}
    ...
    {section("Central de avisos",<><p style={{fontSize:12,color:T.textMuted,marginBottom:10}}>Configure quando o sistema deve alertar.</p><div style={{display:"flex",flexDirection:"column",gap:12}}><label style={{display:"flex",alignItems:"center",flexWrap:"wrap",gap:8}}><input type="checkbox"checked={!!alertConfig.scheduledEnabled}onChange={e=>setAlert("scheduledEnabled",e.target.checked)}style={{accentColor:T.accent}}/><span style={{fontSize:13,color:T.text}}>Alerta quando post agendado passou da data e não foi publicado</span><input type="number"min={0}value={alertConfig.scheduledDaysAfter??1}onChange={e=>setAlert("scheduledDaysAfter",parseInt(e.target.value,10)||0)}style={{width:48,background:T.card,border:`1px solid ${T.border}`,borderRadius:6,padding:"4px 8px",color:T.text,fontSize:12}}/>dias após a data</label><label style={{display:"flex",alignItems:"center",flexWrap:"wrap",gap:8}}><input type="checkbox"checked={!!alertConfig.draftEnabled}onChange={e=>setAlert("draftEnabled",e.target.checked)}style={{accentColor:T.accent}}/><span style={{fontSize:13,color:T.text}}>Alerta quando rascunho está parado há</span><input type="number"min={0}value={alertConfig.draftDaysStale??7}onChange={e=>setAlert("draftDaysStale",parseInt(e.target.value,10)||0)}style={{width:48,background:T.card,border:`1px solid ${T.border}`,borderRadius:6,padding:"4px 8px",color:T.text,fontSize:12}}/>dias</label><label style={{display:"flex",alignItems:"center",flexWrap:"wrap",gap:8}}><span style={{fontSize:13,color:T.text}}>Lembrete de datas especiais com</span><input type="number"min={0}value={alertConfig.specialDatesDaysBefore??12}onChange={e=>setAlert("specialDatesDaysBefore",parseInt(e.target.value,10)||0)}style={{width:48,background:T.card,border:`1px solid ${T.border}`,borderRadius:6,padding:"4px 8px",color:T.text,fontSize:12}}/>dias de antecedência</label><label style={{display:"flex",alignItems:"center",flexWrap:"wrap",gap:8}}><input type="checkbox"checked={!!alertConfig.agendadoMinEnabled}onChange={e=>setAlert("agendadoMinEnabled",e.target.checked)}style={{accentColor:T.accent}}/><span style={{fontSize:13,color:T.text}}>Alerta quando a coluna Agendado tiver menos de</span><input type="number"min={1}value={alertConfig.agendadoMinCards??3}onChange={e=>setAlert("agendadoMinCards",Math.max(1,parseInt(e.target.value,10)||1))}style={{width:48,background:T.card,border:`1px solid ${T.border}`,borderRadius:6,padding:"4px 8px",color:T.text,fontSize:12}}/>cards</label></div></>)}
...
export default function App(){
  ...
  const alertCfg=cms.alertConfig||{};const agendadoMinCards=alertCfg.agendadoMinCards??3;const agendadoAlertOn=alertCfg.agendadoMinEnabled!==false;const agendadoCount=useMemo(()=>filtered.filter(p=>p.column==="agendado").length,[filtered]);
  const showAgendadoLowAlert=agendadoAlertOn&&agendadoCount<agendadoMinCards;
  ...
  return<div data-theme={theme}style={{minHeight:"100vh",background:rootBackground,fontFamily:T.font,color:T.text}}>
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,height:24,background:T.surface,backdropFilter:"blur(10px)",borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",gap:12,fontSize:9,color:T.textDim,zIndex:90}}>
      <span>{posts.length} posts</span><span title="Prontos agora">✓ {readyCount} prontos</span><span>💡{posts.filter(p=>p.column==="ideias_rascunhos").length}</span><span>✍️{posts.filter(p=>p.column==="prod").length}</span><span style={showAgendadoLowAlert?{color:T.red,fontWeight:700}:undefined} title={showAgendadoLowAlert?"Abaixo do mínimo — acelere a produção":undefined}>📅{posts.filter(p=>p.column==="agendado").length}{showAgendadoLowAlert?" ⚠️":""}</span><span>🚀{posts.filter(p=>p.column==="publicado").length}</span><span style={{color:T.border}}>|</span>{lastSaveStatus==="enviando"&&<span style={{color:T.accent}}>Salvando…</span>}{lastSaveStatus==="ok"&&<span style={{color:"#22c55e"}}>Salvo</span>}{lastSaveStatus==="erro"&&<span style={{color:T.red}}>Erro ao salvar (veja acima)</span>}{!isSupabaseConfigured()&&<span title="Configure NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_ANON_KEY para salvar na nuvem" style={{color:T.yellow}}>Salvamento local</span>}<span style={{color:T.border}}>|</span><span><span style={{color:user.color,fontWeight:700}}>{user.name}</span> {user.role==="owner"?"👑":"✏️"}</span>
